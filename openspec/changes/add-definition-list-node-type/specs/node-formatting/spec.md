...
(`heading`, `content`, `footnote`, `image`, `content_group`, `table`, `definition_list`). Container-only nodes
...
#### Scenario: Definition list without format is invalid
- **WHEN** `isValidNode` is given a node of type `definition_list` that has no `format` field
- **THEN** it returns `false`
...
`table` → `MARKDOWN`; `definition_list` → `MARKDOWN`.
...
#### Scenario: Structured blocks reject non-Markdown formats
- **WHEN** `isValidNode` is given a `table` or `definition_list` node with `format: 'TEXT'` or
  `'MARKDOWN_INLINE'`
...
- **THEN** it returns `false`, `canHaveFormat('content', 'MARKDOWN_INLINE')` returns `true`, and
  `canHaveFormat('definition_list', 'MARKDOWN')` returns `true`
...
for `heading`, `content`, `footnote`, `image` and `content_group`, and `MARKDOWN` for the
structured blocks `table` and `definition_list` — the only format they may carry. Tree-construction helpers (`addNodeAfter`,
...
#### Scenario: A node converted to a structured block becomes MARKDOWN
- **WHEN** a `content` node with `format: 'TEXT'` is converted to `table` or `definition_list`
- **THEN** the resulting node has `format: 'MARKDOWN'` and the same `contents`

### Requirement: User can change a node's format from the floating toolbar
When the selection is exactly one content-bearing node **other than `image`, `content_group`,
`table` or `definition_list`**, the system SHALL display a format selector in the `FloatingToolbar` populated only with
...
Although `ALLOWED_FORMATS` permits formats on `image`, `content_group`, `table` and
`definition_list` at the data-model level (validation and import), the UI offers a format selector
for none of them — an image keeps the format assigned at import or creation, a group has no text
whose formatting could matter, and a structured block has exactly one legal format.
...
#### Scenario: Selector is hidden for a selected structured block
- **WHEN** a single `table` or `definition_list` node is selected
- **THEN** no format selector is rendered — their allow-list has exactly one entry
