...
- [x] 9.4 Verify: `npm run test`, `npm run build`, and a manual `npm run dev` smoke (typed `<strong>foo</strong>` does not bold; `**foo**` does; CommonMark autolink `<https://example.com>` still becomes a link; code-fenced `<div>` still shows angle brackets)
